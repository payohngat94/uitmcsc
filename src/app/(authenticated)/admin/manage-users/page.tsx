...
// Firestore services you already had
import { getAllUsers, deleteUser } from "@/lib/firebase/firestore-service";

// NEW: client helpers that call your deployed Cloud Functions
import { approveStudent, setUserRole } from "@/lib/firebase/admin-service.client";
...
const statusConfig: Record<
  UserStatus,
  { icon: React.ElementType; color: string; variant: "default" | "secondary" | "destructive" | "outline" }
> = {
...
export default function ManageUsersPage() {
  ...
  // UPDATED: use callable to set approved flag (also updates claims)
  const handleUpdateStatus = async (docId: string, newStatus: UserStatus) => {
    ...
    const target = users.find((u) => u.docId === docId);
    const uid = target?.uid || docId;

    setIsUpdating((prev) => ({ ...prev, [docId]: true }));
    try {
      const approved = newStatus === "active";
      await approveStudent(uid, approved); // Cloud Function: sets Firestore + custom claim + revokes tokens

      setUsers((prevUsers) =>
        prevUsers.map((user) => (user.docId === docId ? { ...user, status: newStatus } : user))
      );
  ...
  // UPDATED: approve all using callable per user
  const handleApproveAll = async () => {
    setIsApprovingAll(true);
    try {
      const pending = users.filter((u) => u.status === "pending");
      await Promise.all(pending.map((u) => approveStudent(u.uid || u.docId!, true)));
      setUsers((prevUsers) =>
        prevUsers.map((u) => (u.status === "pending" ? { ...u, status: "active" } : u))
      );
      toast({ title: "Success", description: `${pending.length} pending users have been approved.` });
    } catch (error) {
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>
            ...
            <div className="mt-2 text-xs bg-destructive-foreground/10 p-2 rounded">
              ...
              <p>
                In your Firebase project, go to Firestore Database -&gt; Rules and ensure your rules allow admins to
                read the 'users' collection. A common rule is:
              </p>
              <pre className="mt-1 p-1 bg-black/10 rounded font-mono text-[10px]">
  ...
  allow create: if request.auth != null;
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {userToDelete?.email}". This does not delete their authentication record, only their access to this
              application.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Showing {filteredUsers.length} of {users.length} total users.</CardDescription>
        </CardHeader>
        <CardContent>
                      ...
                      <TableRow key={user.docId}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex gap-2 justify-end">
                            ...
                            {/* NEW: Make Admin / Demote buttons */}
                            {user.role !== "admin" && user.docId && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={async () => {
                                  try {
                                    setIsUpdating((p) => ({ ...p, [user.docId!]: true }));
                                    await setUserRole(user.uid || user.docId!, "admin");
                                    setUsers((prev) =>
                                      prev.map((u) => (u.docId === user.docId ? { ...u, role: "admin" } : u))
                                    );
                                    toast({ title: "Role updated", description: `${user.email} is now an admin.` });
                                  } catch (e) {
                                    console.error(e);
                                    toast({ variant: "destructive", title: "Error", description: "Failed to set role." });
                                  } finally {
                                    setIsUpdating((p) => ({ ...p, [user.docId!]: false }));
                                  }
                                }}
                                disabled={isUpdating[user.docId!] || isApprovingAll}
                              >
                            ...
                            {user.role === "admin" && user.docId && (
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    setIsUpdating((p) => ({ ...p, [user.docId!]: true }));
                                    await setUserRole(user.uid || user.docId!, "student");
                                    setUsers((prev) =>
                                      prev.map((u) => (u.docId === user.docId ? { ...u, role: "student" } : u))
                                    );
                                    toast({ title: "Role updated", description: `${user.email} is now a student.` });
                                  } catch (e) {
                                    console.error(e);
                                    toast({ variant: "destructive", title: "Error", description: "Failed to set role." });
                                  } finally {
                                ...
                                disabled={isUpdating[user.docId!] || isApprovingAll}
                              >
                                <UserCog className="mr-1 h-4 w-4" /> Demote
                              </Button>
                            )}
